...
ggsave('figs/stim_regen/contamination_matrix_16S.png',g1)
...
g2<-ggplot(tdat)+
  ...
  scale_fill_manual(values=c("#F8A582","#9ED080","#A37BA7"))
ggsave('figs/stim_regen/contamination_phylum_16S.png',g2)
...
saveRDS(ps, 'data/stim_regen/16Sdada2/phyloseq_dada2.filtered.rds')


#Then do ITS2 
ps <- readRDS('data/stim_regen/ITS2dada2/phyloseq_dada2.rds')
sample_data(ps)$is.neg <- !(sample_data(ps)$Control=='Pos'|is.na(sample_data(ps)$Control) )
...
g1
ggsave('figs/stim_regen/contamination_matrix_ITS2.png',g1)
...
g2
ggsave('figs/stim_regen/contamination_asv_ITS2.png',g2)
...
ps <- prune_taxa(!(taxa_names(ps) %in% unique(gdat$ASV)), ps)
ps <- prune_samples(!(rownames(data.frame(sample_data(ps))) %in% csamps),ps)
saveRDS(ps, 'data/stim_regen/ITS2dada2/phyloseq_dada2.filtered.rds')
