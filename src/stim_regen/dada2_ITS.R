...
#plot qc
for (i in 1:5){
    png(paste0('figs/stim_regen/dada2_fwd_qc_its',i,'.png'))
    print(plotQualityProfile(fnFs[(i*10):(i*10 +3)]))
    dev.off()
    png(paste0('figs/stim_regen/dada2_rev_qc_its',i,'.png'))
...
saveRDS(intout, 'data/stim_regen/ITS2dada2/dada2_errs.rds')


intout <- readRDS('data/stim_regen/ITS2dada2/dada2_errs.rds')
list2env(intout, .GlobalEnv)

##Merge fwd-rev reads
mergers <- mergePairs(dadaFs, filt_fs, dadaRs, filt_rs, verbose=TRUE)
saveRDS(mergers,'data/stim_regen/ITS2dada2/merged_reads_dada2.rds')
...
saveRDS(ps, 'data/stim_regen/ITS2dada2/phyloseq_dada2.rds')
